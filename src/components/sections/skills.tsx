    ...
    >
      ...
      <div className="text-center max-w-3xl mx-auto mb-8 sm:mb-10">
        ...
        <p className="mt-2.5 text-zinc-400 dark:text-zinc-400 text-xs sm:text-sm font-sans">
          {skillsMode === "3d" && enable3D
            ? "Interactive 3D mechanical keyboard in motion — click keycaps or type to test switches."
            : "Interactive 3D mechanical keyboard in motion — select skills or type to trigger keycaps."}
        </p>

        {/* View Mode Toggle Pill: 1st All Skills Grid, then 3D Keyboard Stage */}
        {enable3D && (
          <div className="inline-flex items-center gap-1 mt-6 p-1 rounded-full border backdrop-blur-md transition-colors bg-zinc-950/80 border-zinc-800/80 shadow-lg pointer-events-auto">
            <button
      ...
      {skillsMode === "3d" && enable3D ? (
        <div className="flex flex-col items-center justify-between min-h-[55vh] sm:min-h-[62vh] relative pointer-events-none">
          {/* Top Hint Badge */}
          <div className="flex items-center gap-2 px-4 py-2 rounded-full border text-[11px] font-mono backdrop-blur-md border-zinc-800/90 bg-zinc-900/40 text-zinc-300 pointer-events-auto shadow-sm">
            <Sparkles className="w-3.5 h-3.5 text-emerald-400" />
            <span>Interactive 3D mechanical keyboard — click keycaps or type to explore</span>
          </div>
            ...
            {displayInspected ? (
              <div
                className={`p-4 sm:p-5 rounded-2xl border backdrop-blur-md shadow-2xl flex items-center gap-4 transition-all duration-300 animate-in fade-in slide-in-from-bottom-2 ${
                  theme === "dark"
                    ? "border-zinc-800/90 bg-zinc-900/40 hover:bg-zinc-900/70 hover:border-zinc-700 text-zinc-100"
                    : "border-zinc-200 bg-white/80 hover:bg-white hover:border-zinc-300 shadow-sm text-zinc-900"
                }`}
              >
              ...
              <div
                className={`p-3 sm:p-4 rounded-2xl border backdrop-blur-md text-center flex items-center justify-between gap-4 ${
                  theme === "dark"
                    ? "border-zinc-800/90 bg-zinc-900/40 text-zinc-300"
                    : "border-zinc-200 bg-white/80 text-zinc-700 shadow-sm"
                }`}
              >
                ...
                <button
                  onClick={() => setSkillsMode("grid")}
                  className="flex items-center gap-1 text-xs font-semibold text-white hover:text-emerald-400 px-3 py-1.5 rounded-lg border border-zinc-700 bg-zinc-800/80 transition-colors whitespace-nowrap shrink-0"
                >
                  <span>Full Grid</span>
        ...
        /* VIEW 2: Complete Skills Grid (Cards with frosted blur matching experience & education) */
        <div className="space-y-6 animate-in fade-in duration-300 pointer-events-auto">
          {/* Controls: Search & Category Filter */}
          <div className="flex flex-col md:flex-row items-center justify-between gap-4">
            {/* Category Filter Pills with blur */}
            <div className="flex flex-wrap items-center justify-center md:justify-start gap-1.5 w-full md:w-auto">
              {categories.map((cat) => (
                <button
                  key={cat}
                  onClick={() => setActiveCategory(cat)}
                  className={`px-3.5 py-1.5 rounded-full text-xs font-medium backdrop-blur-md transition-all duration-200 border ${
                    activeCategory === cat
                      ? theme === "dark"
                      ...
                      : theme === "dark"
                      ? "bg-zinc-900/40 border-zinc-800/80 text-zinc-400 hover:text-white hover:border-zinc-700 hover:bg-zinc-900/70"
                      : "bg-white/80 border-zinc-200 text-zinc-600 hover:text-zinc-900 hover:border-zinc-300 hover:bg-white"
                  }`}
                >
                  {cat}
                </button>
            ...
            <div className="relative w-full md:w-64">
              ...
              <input
                ...
                className={`w-full pl-9 pr-3 py-1.5 text-xs rounded-xl border backdrop-blur-md focus:outline-none transition-colors ${
                  theme === "dark"
                    ? "bg-zinc-900/40 border-zinc-800/80 text-zinc-200 placeholder-zinc-500 focus:border-zinc-600 focus:bg-zinc-900/70"
          ...
          {/* Full Grid of Frosted Blur Skill Cards matching Experience & Education sections */}
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3 sm:gap-4">
            {filteredSkills.map((skill) => {
              const isSelected = displayInspected?.name === skill.name;
              return (
                <div
                  key={skill.name}
                  onClick={() => {
                  ...
                  className={`group relative flex flex-col items-center justify-center gap-2.5 rounded-2xl p-4 sm:p-5 border backdrop-blur-md transition-all duration-200 cursor-pointer overflow-hidden ${
                    isSelected
                      ? theme === "dark"
                        ? "border-zinc-300 bg-zinc-800/70 shadow-xl scale-[1.03]"
                        : "border-zinc-700 bg-zinc-100/90 shadow-lg scale-[1.03]"
                      : theme === "dark"
                      ? "border-zinc-800/90 bg-zinc-900/40 hover:bg-zinc-900/70 hover:border-zinc-700 hover:-translate-y-0.5"
                      : "border-zinc-200 bg-white/80 hover:bg-white hover:border-zinc-300 shadow-sm hover:-translate-y-0.5"
                  }`}
                >
                  {/* Skill Icon */}
          ...
          {/* Selected Skill Detail Modal/Card with frosted blur */}
          {displayInspected && (
            <div
              className={`p-4 sm:p-5 rounded-2xl border backdrop-blur-md shadow-2xl flex items-center gap-4 animate-in fade-in slide-in-from-bottom-2 ${
                theme === "dark"
                  ? "border-zinc-800/90 bg-zinc-900/50 hover:bg-zinc-900/70 hover:border-zinc-700 text-zinc-100"
                  : "border-zinc-200 bg-white/90 hover:bg-white hover:border-zinc-300 text-zinc-900 shadow-md"
              }`}
            >
              <div className="w-12 h-12 rounded-xl bg-zinc-800/80 border border-zinc-700 flex items-center justify-center shrink-0 p-2">
                <img
